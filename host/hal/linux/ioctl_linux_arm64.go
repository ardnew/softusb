...
// arm64 uses the same sizes as amd64 (LP64 model).
const (
	sizeofCtrlTransfer = 24 // struct usbdevfs_ctrltransfer
	sizeofBulkTransfer = 24 // struct usbdevfs_bulktransfer (with padding)
	sizeofInt          = 4
	sizeofPointer      = 8
)

// Usbdevfs ioctl numbers for arm64.
